import { Avatars, Databases, Account, Client, ID, Query, Functions, Storage } from 'react-native-appwrite';
    ...
    export const createUser = async (email, password, username) => {
     ...
     export const signIn = async (email, password) => {
      try {
          ...
          // // Delete each session if there are any
          // if (sessions.total > 0) {
...
export const deleteToiletEvents = async () => {
  try {
    await databases.deleteDocument(
      databaseId,
      toiletEventsCollectionId,
...
export const createAlerts = async (title, date, time
  ...
  try {
    const createAlert = await databases.createDocument(
      databaseId,
      alertsCollectionId,
      ID.unique(),
      {
