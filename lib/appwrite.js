...
export const updateUserBlockedList = async (userId, blockedUsername) => {
  try {
    ...
    const { BlockedUsers = [] } = userDocument;

    // Add the blocked user if not already in the list
    if (!BlockedUsers.includes(blockedUsername)) {
      BlockedUsers.push(blockedUsername);
    }

    // Update the user document with the new BlockedUsers array
