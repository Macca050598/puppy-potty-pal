...
export const getDogToiletEvents2 = async (dogId, limit = 20, type = null) => {
  try {
    const filters = [
      Query.equal('associatedDog', dogId),
      Query.orderDesc('$createdAt'),
...
export const addToiletEvent = async (dogId, type, location, timestamp = new Date().toISOString()) => {
  ...
  export const updateToiletEvent = async (eventId, updatedEvent) => {
    ...
    export const getAllDogsToiletEvents = async (dogIds, limit = 20) => {
      try {
        const events = await databases.listDocuments(
          databaseId,
          toiletEventsCollectionId,
          [
            Query.equal('associatedDog', dogIds),
            Query.orderDesc('$createdAt'),
            Query.limit(limit)
          ]
        );
        ...
        console.error("Error fetching dogs' toilet events:", error);
        throw error;
      }
    };
...
// Add these functions to your existing Appwrite configuration file
...
export const updateFamilyMembers = async (familyId, selectedMembers) => {
  try {
    ...
    // Update family_ids for each user
    const allUsers = await getAllUsers();
    for (const user of allUsers) {
      if (selectedMembers[user.$id]) {
        // User should be in the family
        if (!user.family_ids.includes(familyId)) {
          await updateUserFamilies(user.$id, familyId);
        }
      } else {
        // User should not be in the family
        if (user.family_ids.includes(familyId)) {
          await leaveFamily(user.$id, familyId);
        }
      }
    }
...
export const updateFamilyDogs = async (familyId, selectedDogs) => {
  try {
    // Get all user dogs
    const allUserDogs = await getUserDogs();

    // Get current family dogs
    const currentFamilyDogs = await getFamilyDogs(familyId);

    for (const dog of allUserDogs) {
      ...
      if (shouldBeInFamily && !isCurrentlyInFamily) {
        ...
        await databases.updateDocument(
          ...
          dogCollectionId,
          dog.$id,
          { families: familyId } // Set the relationship to the family ID
        );
      } else if (!shouldBeInFamily && isCurrentlyInFamily) {
        // Remove dog from family
        await databases.updateDocument(
          databaseId,
          dogCollectionId,
          dog.$id,
          { families: null } // Remove the relationship by setting it to null
        );
      }
    }
    ...
    console.error("Error updating family dogs:", error);
    throw error;
  }
};
...
export const joinFamily = async (userId, familyCode) => {
  try {
    ...
    // Add the family to the user's family_ids
    await updateUserFamilies(userId, family.$id);
...
export const getUserAndFamilyDogs = async (userId) => {
  try {
    // Fetch user's personal dogs
    const personalDogs = await getUserDogs(userId);
    ...
    // Flatten the array of family dogs and mark as not owned by user
    const familyDogs = familyDogsArrays.flat().map(dog => ({ ...dog, isOwnedByUser: dog.creator === userId }));

    // Combine personal and family dogs
    const allDogs = [...personalDogsWithOwnership, ...familyDogs];

    // Remove duplicates (in case a personal dog is also a family dog)
