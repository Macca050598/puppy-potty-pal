...
export const getDogToiletEvents = async (dogId, limit = 50) => {
  try {
    ...
    //   console.log("First event structure:", JSON.stringify(events.documents[0], null, 2));
    // } else {
    //   console.log("No events found");
    // }
    
    // For now, let's filter the events in JavaScript
    return events.documents.filter(event => event.dogId === dogId);
  } catch (error) {
...
export const addToiletEvent = async (dogId, type, location, notes, timestamp = new Date().toISOString()) => {
    try {
      const newEvent = await databases.createDocument(
        ...
        ID.unique(),
        {
          associatedDog: dogId,  // or associatedDog.$id if it's a relationship field
          type: type,
